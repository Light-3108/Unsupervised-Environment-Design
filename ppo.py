...
def reset_lstm_hidden_for_done_envs(hidden, done_mask, device):
    """
    Reset LSTM hidden states for environments that are done.
    
    Args:
        hidden: Tuple of (h, c) LSTM hidden states with shape (1, num_envs, hidden_size)
        done_mask: Boolean tensor indicating which environments are done
    ...
    if done_mask.any():
        h, c = hidden
        h = h.clone()  # Clone to avoid in-place operations
        c = c.clone()
        h[:, done_mask, :] = 0.0
        c[:, done_mask, :] = 0.0
        # Random initialization for reset environments
        # h[:, done_mask, :] = torch.randn_like(h[:, done_mask, :]) * 0.1
        # c[:, done_mask, :] = torch.randn_like(c[:, done_mask, :]) * 0.1
        return (h, c)
    return hidden
...
def ppo_update(data_buffer, ppo_epochs, clip_param):
    for _ in range(ppo_epochs):
        for data_batch in data_buffer:
            ...
            new_dist, new_value = rl_model.forward_no_lstm(data_batch["states"], data_batch["directions"])
...
while frames_seen < max_frames:
    ...
    hidden = (torch.zeros(1, num_envs, 256).to(device), torch.zeros(1, num_envs, 256).to(device))
    print("Rollout!")
    with torch.no_grad():  # Don't need computational graph for roll-outs
        while step < num_steps:
            ...
            dist, value, hidden = rl_model(state, direction, hidden)  # Forward pass of actor-critic model
            ...
            if done.any():
                # Create mask for environments that are done
                done_mask = torch.tensor(done, dtype=torch.bool, device=device)
                # Reset hidden states for done environments using helper function
                hidden = reset_lstm_hidden_for_done_envs(hidden, done_mask, device)
        ...
        _, next_value, _ = rl_model(state, direction, hidden)
